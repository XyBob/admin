...
function authCheck($rule)
{
    ...
    if(in_array(strtolower($rule), session('action'))){
        return true;
    }
    return false;
}
 ...
 * 保存操作日志到数据库
 */
function saveOperationLog($type,$before_vars='',$after_vars='',$desc='',$sql)
{
    $AdminOperationRecordModel = new \app\admin\model\AdminOperationRecordModel();
    $data = [
        'user_id'=>session('id'),
        'type'=>$type,
        'before_vars'=>isset($before_vars)?is_array($before_vars)?json_encode($before_vars):$before_vars:'',
        'after_vars'=>isset($after_vars)?is_array($after_vars)?json_encode($after_vars):$after_vars:'',
        'desc'=>$desc,
        'create_time'=>time(),
        'sql'=>$sql,
        'action_name'=>lcfirst(request()->controller()),
        'method'=>lcfirst(request()->action()),
    ];
    $AdminOperationRecordModel->addRecord($data);
}
